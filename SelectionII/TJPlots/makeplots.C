...
float tot_on = 216474; //31403; // 525613; //341100; //341493; // 547616
float tot_off = 191125; //22369 ; //379202; //377273; //370896; // 382718
float tot_bnbcos = 93250; //38850; //101600; //101850;  // 191362
...
void plot(TFile *f, std::string name0){
  ...
  if (fabs(h[0][0]->GetMean()-(h[0][0]->GetXaxis()->GetXmin() + (h[0][0]->GetXaxis()->GetXmax() - h[0][0]->GetXaxis()->GetXmin())/2))<0.05*(h[0][0]->GetXaxis()->GetXmax() - h[0][0]->GetXaxis()->GetXmin())){
    ...
    leg->AddEntry(h[2][2], "Other Background","f");
